...
下面还会写一些自己做过的工具，不会涉及具体实现（当然欢迎交流），也有很多是从别的项目抄的。
我对提升大家的工作效率还是很有兴趣的，也可以算入“优化”这一标题下

### redis

#### 内存占用高

背景是一个实例占了9G，理想情况6G以内

1. bgsave, scp，把dump.rdb拷出来
2. [rdr-darwin](https://github.com/xueqiu/rdr) show dump.rdb

得到的结果业务key比较多，可能有点敏感，就不放在这里了。

优化完还剩4G，大概就是每个模块看了看，删除了一些东西，增加了一些过期机制等

#### 内存突增

我并没有真的解决这个问题，目前暂定解决方案是换机器

问题：每天11点（正好是我们重写aof的时间）redis内存暴涨，总内存40G的机器，能涨上去16G，可用最低会从20G跌到4G

我按照这个顺序排查：
1. 复现

首先要排除11点有其他结算脚本之类的影响，另找了一个时间跑，也复现
再排除是连续写多个实例导致，只跑一个，也复现
所以就是对于任意大redis，bgrewriteaof，都能复现

所以问题是：bgrewriteaof，在重写子进程退出的十秒到两分钟之内，整台机器内存会暴涨，但是每个实例的占用都是正常的
...
更啥都没看出来，高点没什么高内存占用的可疑进程

4. vmstat

持续采样看看哪里跌了，发现free暴跌，buffer、cache没怎么变化

5. atop

slab正常，再观察高点有一个jbd-sda1-8 IO占用很高
pidstat -d也能看出jbd这个东西有点异常，有一些iodelay，但他明面没有占用，不确定是不是他

6. 看代码
我又去把redis aof.c重新看了一遍，没找到嫌疑人

到这里运维说换机器解决，先这样，如果将来还有问题，那就深入了解一下内核任务再来排查

### 前端工具
有时候前端需要一些工具，帮着写一点

#### 打包脚本优化
以前项目打包要20分钟（即使是大版本之后缝缝补补的小版本），我检查了一下脚本发现有点问题，整体优化了一遍之后4分钟以内

1. C# StringBuilder，占用较多内存，sb这个东西ToString会生成一份拷贝，而且是倍增申请内存，导致我们269M的.h文件，直接会申请1G内存。我们32位64位一起打，Unity每个实例多花2G冤枉内存
2. C# Process，我们以前每个lua文件起一个luajit，很卡，因为C# Process要准备很多参数，给他改成了一个脚本jit_all，c#调用这个jit_all

看上去这两个优化微不足道，但是做完之后确实Unity内存降下去了，打包速度涨上来了。

#### 远程调试

有时候有一些玩家本地的Bug，登录玩家账号不能复现，或者设备是Ios 没有adb，看不到输出，或者忘记了怎么操作的，很难复现，可能要看本地日志文件。

1. 思路
后端推送一段代码给前端执行
后端在任意mid之后，推送一段文本给前端
前端调用`loadstring(s)()`，执行后端传过来的代码，再通过协议的方式发回给后端，或者上传文件的方式上传日志文件
这个调用loadstring是有上下文的，所以理论上内网写lua能解决的东西，都可以远程给他解决
2. 现有功能
普通打印，这个比较简单，按照上面的思路执行代码就好了
获取日志，这个也比较简单，读取文件内容，读出来就变成上一条了
远程打断点，这个我写完之后只试过一次，上面的普通打印实测能解决95%的问题。原理是发送一段debug.sethook，执行到某个文件某一行的时候打印钩子，然后可以定制一些执行若干次或者什么条件就上传文件到远程主机的逻辑

#### 跑表工具

这个是总有策划反馈跑表太慢，结合其他项目的跑表做了一套我们项目的
其实就是很简单的给他换成多进程的跑表
multiprocessing.Pool
效果很不错，大约节省了70%-80%的时间，后端我也做了同样改动

#### Unity工具

抄的其他项目的，就是Unity上面运行暂停下一帧之外，定制一些按钮，比如跑表，快速重启，打开GM指令面板等等

#### 前端报错机器人

定期拉取报错 + git blame + 办公群提醒一条龙

主要用于测试服及早发现问题+更新版本之后及早发现问题
...
后面我主要是写的后端的，前端的我写了一点，后面被伦伦改的面目全非

主要是检查lua文件是不是有语法报错，检查相应文件是否提交了meta

以及根据username做一些权限限制

拉取之后 git config core.hookpath设置一下就行，给他放到项目的跑表文件里，他就一定会跑了
