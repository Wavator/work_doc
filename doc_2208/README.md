...
- 家具效果在一些战斗模块中完全失效
    ...
    ![](assets/16619245725326.jpg)

    左边为优化前，右边为优化后

    指标为CPU idle time
    
    优化前发现，我们gate机器除了每天八点CPU会掉之外，每小时的15分会有五分钟的时间一个核心跑满
    
    排查下来发现是日志文件太大了，我们会有脚本检查gate机器过去一段时间的log。
    
    我们gate是起了八个tornado的多路复用器，理想情况下每个进程占一个核心，如果五分钟的时间里有个grep把一个CPU跑满了，就会导致进程上下文切换，消耗就会比较大，玩家很多的情况下就会卡顿

    日志grep大概是wc + tail + grep（tail+grep用来匹配增量）
    
    wc这一步会被日志大小影响很多，所以在每天分割日志之前，核心占满的时间都是递增的
    
    排查日志大小为什么这么大（其实也不用排查，线上开的info级别的日志是以前就知道的）
    
    关掉了info级别的日志，部分需要看的情况（比如f000）改为warning
    
    优化完后CPU就好了。
    
    日志级别以后要注意，这个是从以前项目拿过来的，以后自己写类似代码也要注意。
    
    ps：CPU上下文切换，看看某个gate的进程号，然后`pidstat -wu 2`
